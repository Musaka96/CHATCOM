...
export const PRICE_CENTS = Number(process.env.PRICE_CENTS || process.env.NEXT_PUBLIC_PRICE_CENTS || 1990);

// Single source of truth for the displayed price, e.g. "$19.90". Every place
// the price is shown on the site must use this so it stays consistent with
// checkout — a hard requirement for Paddle's domain review.
export const PRICE_DISPLAY = `$${(PRICE_CENTS / 100).toFixed(2).replace(/\.00$/, "")}`;
